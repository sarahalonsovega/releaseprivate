...
export function ContactDialog({ open, onClose }) {
  ...
  if (!open) return null;
  return (
    <div className="contact-dialog-backdrop" role="presentation" onMouseDown={event => event.target === event.currentTarget && onClose()}>
      <section ref={dialogRef} className="contact-dialog" role="dialog" aria-modal="true" aria-labelledby="contact-dialog-title">
        <button className="contact-dialog-close" type="button" onClick={onClose} aria-label="Close contact form">×</button>
